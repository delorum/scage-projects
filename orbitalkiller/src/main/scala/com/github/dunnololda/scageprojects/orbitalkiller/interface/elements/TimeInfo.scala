...
import com.github.dunnololda.scageprojects.orbitalkiller_cake.TimeConstants.{base_dt, k, realtime}
...
class TimeInfo extends InterfaceElement {
  ...
  override protected def _update(): Unit = {
    val time_acceleration = f"x${(Main.timeMultiplier * k).toInt} (${1f * Main.timeMultiplier / 63 * Main.tps}%.2f)"
    if (Main._stop_after_number_of_tacts > 0 || Main._set_stop_time) {
      ...
      if (Main.timeMultiplier != realtime) {
        if (Main._set_stop_time) {
          strings_with_stop_moment(1) =
            s"[gОстановка через ${timeStrSec((Main._stop_after_number_of_tacts * base_dt * 1000).toLong)} (${timeStrSec(
                (Main._stop_after_number_of_tacts * base_dt * 1000 / (1f * Main.timeMultiplier / 63 * Main.tps)).toLong
              )})]"
        } else {
          strings_with_stop_moment(1) =
            s"Остановка через ${timeStrSec((Main._stop_after_number_of_tacts * base_dt * 1000).toLong)} (${timeStrSec(
                (Main._stop_after_number_of_tacts * base_dt * 1000 / (1f * Main.timeMultiplier / 63 * Main.tps)).toLong
              )})"
        }
      } else {
        if (Main._set_stop_time) {
          strings_with_stop_moment(1) =
            s"[gОстановка через ${timeStrSec((Main._stop_after_number_of_tacts * base_dt * 1000).toLong)}]"
        } else {
          strings_with_stop_moment(1) =
            s"Остановка через ${timeStrSec((Main._stop_after_number_of_tacts * base_dt * 1000).toLong)}"
        }
      }
