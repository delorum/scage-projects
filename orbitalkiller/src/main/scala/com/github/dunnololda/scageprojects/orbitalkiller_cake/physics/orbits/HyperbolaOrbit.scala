...
class HyperbolaOrbit(
  ...
  val r_p: Double = a * (e - 1)
  // перигей
  val p: Double = a * (e * e - 1) // фокальный параметр (половина длины хорды, проходящей через фокус и перпендикулярной к фокальной оси)

  val teta_deg_min: Double = 180 - (1 / e).myacos / math.Pi * 180
  // разрешенные углы: от 0 до этого угла
  val teta_rad_min: Double = teta_deg_min / 180.0 * math.Pi

  val teta_deg_max: Double = -180 + (1 / e).myacos / math.Pi * 180 + 360
  // разрешенные углы: от этого угла до 360
  val teta_rad_max: Double = teta_deg_max / 180.0 * math.Pi
  ...
  val f_minus_center_n: DVec = (f - center).n
  val half_center: DVec = f - f_minus_center_n * (center.dist(f) * 0.5)
  val inv_n: Double = a * math.sqrt(a / mu)
  ...
  def tetaSignedRadByDir(dir: DVec): Double = f_minus_center_n.signedRad(dir)
