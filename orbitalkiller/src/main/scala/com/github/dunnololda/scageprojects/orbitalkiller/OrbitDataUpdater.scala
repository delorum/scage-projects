...
object OrbitDataUpdater {
  ...
  private def printCalculatedData(
      flight_time_msec: Long,
      orbital_point: DVec,
      mouse_teta_rad2Pi: Double,
      o: KeplerOrbit,
      planet_radius: Double,
      orbit_color: ScageColor): Unit = {
    ...
    openglLocalTransform {
      ...
      print(
        s"  $flight_time_str : ${mOrKmOrMKm(o.distanceByTrueAnomalyRad(mouse_teta_rad2Pi) - planet_radius)} : ${msecOrKmsecOrKmhour(vnorm)}",
        Vec.zero,
  ...
  private def hyperbolaOrbitDataForNonPlayerShip(
      update_count: Long,
      bs: MutableBodyState,
      body_radius: Double,
  ...
  private def hyperbolaOrbitDataForPlanet(
      update_count: Long,
      bs: MutableBodyState,
      body_radius: Double,
      planet_state: MutableBodyState,
  ...
  private def hyperbolaOrbitDataForPlayerShip(
      update_count: Long,
      bs: MutableBodyState,
      body_radius: Double,
      planet_state: MutableBodyState,
    ...
    OrbitRenderData(
      ...
      ccw,
      () => {
        val real_trajectory_enabled =
          interfaceHolder.realTrajectorySwitcher.showRealTrajectory && RealTrajectory.realTrajectory.nonEmpty
        if (real_trajectory_enabled) {
          ...
          if (_stop_after_number_of_tacts > 0) {
            drawFilledCircle(
              new_o.orbitalPointByTrueAnomalyRad(_stop_in_orbit_true_anomaly) * scale,
              3 / globalScale,
              RED
            )
            drawFilledCircle(
          ...
          if (new_o.tetaRad2PiValid(mouse_teta_rad2Pi)) {
            val away_from_rp =
              (bs.coord - new_o.f) * (bs.vel - planet_state.vel) >= 0 // приближаемся к перигею или удаляемся от него?
  ...
  private def ellipseOrbitDataForNonPlayerShip(
      update_count: Long,
      bs: MutableBodyState,
      body_radius: Double,
      planet_state: MutableBodyState,
  ...
  private def ellipseOrbitDataForPlanet(
      update_count: Long,
      bs: MutableBodyState,
      body_radius: Double,
      planet_state: MutableBodyState,
  ...
  private def ellipseOrbitDataForPlayerShip(
      update_count: Long,
      bs: MutableBodyState,
      body_radius: Double,
      planet_state: MutableBodyState,
  ...
  def updateOrbitData(
      ...
      need_planets: Set[Int],
      calculate_orbit_around: Option[Int]): Option[OrbitRenderData] = {
    val celestials = some_system_state
      .filter(kv => need_planets.contains(kv._1))
      .flatMap(kv => {
    ...
    val bs_ang = bs.ang
    calculate_orbit_around
      .map(idx => celestials.find(_._1.index == idx))
      .getOrElse(insideSphereOfInfluenceOfCelestialBody(bs_coord, bs_mass, celestials)) match {
      case Some((planet, planet_state)) =>
        ...
        orbit match {
          case h: HyperbolaOrbit =>
            val yy = (-(-1.0 / h.e).myacos + 0.1 to (-1.0 / h.e).myacos - 0.1 by 0.1)
              .map(true_anomaly => {
            ...
            if (bs.index != player_ship.thisOrActualProxyShipIndex) {
              if (shipsHolder.shipIndicies.contains(bs.index)) {
                Some(
                  hyperbolaOrbitDataForNonPlayerShip(
                    update_count,
                    bs,
                    body_radius,
                    planet_state,
              ...
              Some(
                hyperbolaOrbitDataForPlayerShip(
                  update_count,
                  bs,
                  body_radius,
                  planet_state,
          ...
          case e: EllipseOrbit =>
            if (bs.index != player_ship.thisOrActualProxyShipIndex) {
              if (shipsHolder.shipIndicies.contains(bs.index)) {
                Some(
                  ellipseOrbitDataForNonPlayerShip(
                    update_count,
                    bs,
                    body_radius,
                    planet_state,
                ...
                Some(
                  ellipseOrbitDataForPlanet(update_count, bs, body_radius, planet_state, planet, e, ccw, orbit_color)
                )
              }
            } else {
              Some(
                ellipseOrbitDataForPlayerShip(update_count, bs, body_radius, planet_state, planet, e, ccw, orbit_color)
              )
            }
        }
      case None => None
