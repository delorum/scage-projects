...
class MoonRelativeInfo(degOrKm: DegOrKm) extends InterfaceElement {
  ...
  override protected def _update(): Unit = {
    ...
    if (player_ship.isLandedOnMoon) {
      ...
      val ship_moon_vertical_speed_str = msecOrKmsecOrKmhour((player_ship.linearVelocity - moon.linearVelocity) * (player_ship.coord - moon.coord).n)
      val ship_moon_tangent_speed_str = msecOrKmsecOrKmhour(((player_ship.linearVelocity - moon.linearVelocity) * (player_ship.coord - moon.coord).p) / player_ship.coord.dist(moon.coord) * moon.radius - moon.groundSpeedMsec)
