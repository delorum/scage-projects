...
class MoonRelativeInfo extends InterfaceElement {
  ...
  override protected def _update(): Unit = {
    ...
    val ship_moon_vertical_speed = (ship.linearVelocity - moon.linearVelocity) * (ship.coord - moon.coord).n
    val ship_moon_tangent_speed = ((ship.linearVelocity - moon.linearVelocity)*(ship.coord - moon.coord).p)/ship.coord.dist(moon.coord)*moon.radius - moon.groundSpeedMsec
    val ship_moon_position = f"${correctAngle(DVec(0, 1).deg360(ship.coord - moon.coord) - moon.currentState.ang)}%.3f град."
    if((ship.coord.dist(moon.coord) - moon.radius) < ship.radius && ship_moon_vertical_speed.abs < 0.5 && ship_moon_tangent_speed.abs < 0.5) {
      strings(0) = s"Луна: dist=${mOrKmOrMKm(ship.coord.dist(moon.coord) - moon.radius)}, landed, pos=$ship_moon_position"
    } else {
      val ship_moon_vertical_speed_str = msecOrKmsec(ship_moon_vertical_speed)
      val ship_moon_tangent_speed_str = msecOrKmsec(ship_moon_tangent_speed)
