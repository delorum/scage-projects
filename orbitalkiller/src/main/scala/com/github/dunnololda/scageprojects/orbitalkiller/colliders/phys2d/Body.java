...
public strictfp class Body {
     ...
     */
    void configureRestingBodyDetection(double hitTolerance, double rotationTolerance, double positionTolerance) {
...
//				    && (force.lengthSquared() < positionTolerance)
     ...
     */
    public void adjustPosition(ROVector2f delta, double scale) {
        lastPosition.set(position);
     ...
     */
    public void adjustRotation(double delta) {
        rotation += delta;
    }
     ...
     * @param delta The amount to change the velocity by
     */
    public void adjustAngularVelocity(double delta) {
        if (!isRotatable()) {
            return;
        }
     ...
     */
    public double getBiasedAngularVelocity() {
     ...
     * @param delta The amount to change the velocity by
     */
    public void adjustBiasedAngularVelocity(double delta) {
        if (!isRotatable()) {
            return;
        }
    ...
    public double getEnergy() {
        double velEnergy = getMass() * getVelocity().dot(getVelocity());
        double angEnergy = getI() * (getAngularVelocity() * getAngularVelocity());
     ...
     */
    public void setMaxVelocity(double maxX, double maxY) {
