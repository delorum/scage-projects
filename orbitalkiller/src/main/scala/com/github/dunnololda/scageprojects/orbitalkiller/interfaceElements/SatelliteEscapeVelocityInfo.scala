    ...
    insideSphereOfInfluenceOfCelestialBody(ship.coord, ship.mass, currentPlanetStates) match {
      case Some((planet, planet_state)) =>
        ...
        if(ship_speed < sat_speed) {
          strings(0) = f"Скорость корабля/спутника/убегания: $planet_name [c${msecOrKmsec(ship_speed)}]/${msecOrKmsec(sat_speed)}/${msecOrKmsec(esc_speed)}"
        } else if(ship_speed < esc_speed) {
          strings(0) = f"Скорость спутника/корабля/убегания: $planet_name ${msecOrKmsec(sat_speed)}/[c${msecOrKmsec(ship_speed)}]/${msecOrKmsec(esc_speed)}"
        } else {
          strings(0) = f"Скорость спутника/убегания/корабля: $planet_name ${msecOrKmsec(sat_speed)}/${msecOrKmsec(esc_speed)}/[c${msecOrKmsec(ship_speed)}]"
        }
      case None =>
        strings(0) = ""
    }
