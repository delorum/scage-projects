...
import com.github.dunnololda.scageprojects.orbitalkiller_cake.util.StringFormatUtils.gOrKg

class EnginesInfo(playerShip: Ship4) extends InterfaceElement {
  ...
  override protected def _update(): Unit = {
    ...
    val engines_str = s"${
      if (playerShip.isAlive) {
        playerShip.engines.filter(e => e.active || playerShip.isSelectedEngine(e)).map(e => {
          if (playerShip.isSelectedEngine(e)) {
            if (e.active) {
              engines_active = true
        ...
        playerShip.engines.map(e => {
          f"${e.name}: ${e.power / 1000}%.1f кН (${e.workTimeStr})"
        }).mkString(", ")
      }
    }${val x = playerShip.engines.filter(e => e.active).map(e => e.fuelConsumptionPerSec).sum; if (x != 0) f" (${gOrKg(x)} в сек)" else ""}"
  ...
  override def color: ScageColor = {
    if (isMinimized && playerShip.engines.exists(_.active)) ScageColor.RED else ScageColor.YELLOW
  }
