...
abstract class Satellite1(
  ...
  extends PolygonShip(index, "Светлячок", init_coord, init_velocity, init_rotation, ship_designer, true) {
  ...
  val docking_points: List[DockingPoints] = List(
    new DockingPoints(
      ...
      DVec(1.5, -16),
      this,
      Some(DisabledEngine(two.index)),
      createOrderedHull(List(1 -> 20)) ::: List(points.head)
    )
  )
