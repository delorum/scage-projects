...
abstract class ProxyShip(
  ...
  override lazy val convex_parts: List[PolygonShape] = {
    ship1.convex_parts.map(p => p.copy(points = p.points.map(_ + ship1_coord_diff))) :::
      ship2.convex_parts.map(p =>
        p.copy(points = p.points.map(p => p.rotateDeg(ship2_rotation_diff) + ship2_coord_diff))
      )
  }
  ...
  override def nearestFreeDockingPoints(coord: DVec): Option[DockingPoints] = {
    nonProxyShips
      .flatMap(s => s.docking_points.filter(dp => !s.dockData.exists(dd => dd.our_dp.index == dp.index)))
      .sortBy(_.curP1.dist(coord))
      .headOption
  }
