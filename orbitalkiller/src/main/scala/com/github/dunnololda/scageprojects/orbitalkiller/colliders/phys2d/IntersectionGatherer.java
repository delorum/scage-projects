...
public class IntersectionGatherer {
    ...
    public void intersect(int a, int b) {
        ...
        dist.sub(startA);
        double distFromVertA = dist.lengthSquared();
        dist = new Vector2f(position);
        dist.sub(startB);
        double distFromVertB = dist.lengthSquared();
    ...
    class SortableIntersection extends Intersection {
         ...
         */
        public SortableIntersection(int edgeA, int edgeB, Vector2f position, boolean isIngoing, double distFromVertA, double distFromVertB) {
