...
class PlanetsInfluenceInfo extends InterfaceElement {
  ...
  override protected def _update(): Unit = {
    val sun_force = gravityForce(sun.coord, sun.mass, ship.coordOrFirstPartCoord, ship.mass, G).norma
    val earth_force = gravityForce(earth.coord, earth.mass, ship.coordOrFirstPartCoord, ship.mass, G).norma
    val moon_force = gravityForce(moon.coord, moon.mass, ship.coordOrFirstPartCoord, ship.mass, G).norma
