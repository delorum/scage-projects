...
class PlanetWithAir(
  ...
  def velocityRelativeToAir(
      ship_coord: DVec,
      ship_velocity: DVec,
      planet_coord: DVec,
      planet_velocity: DVec,
      planet_ang_vel: Double): DVec = {
    (ship_velocity - planet_velocity) - (ship_coord - planet_coord).p * (planet_ang_vel.toRad * ship_coord.dist(
      planet_coord
    ))
  }
  ...
  // C - Drag coefficient
  private def airResistance(
      v: DVec,
      h: Double,
      A: Double,
      C: Double /*, check_obscure_func: DVec => Boolean*/): DVec = {
    val T = temperature(h)
    if (T <= 0) DVec.zero
    else {
  ...
  private def airResistance(
    ...
    airResistance(
      velocityRelativeToAir(ship_coord, ship_velocity, planet_coord, planet_velocity, planet_ang_vel),
