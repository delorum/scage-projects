...
class EvolutionHelper(mutable_system: mutable.HashMap[Int, MutableSystemPart]) {
  ...
  def gravityForceFromTo(planet_index: Int, ship_index: Int): DVec = {
    ...
    } yield gravityForce(planet.coord, planet.mass, ship.coord, ship.mass)).getOrElse(DVec.zero)
  }
  ...
  def bodyStates(indicies: collection.Set[Int]): Seq[MutableBodyState] =
    mutable_system.filter(kv => kv._2.body.active && indicies.contains(kv._1)).map(kv => kv._2.body).toSeq

  def bodyStatesMap(indicies: Set[Int]): mutable.Map[Int, MutableBodyState] =
