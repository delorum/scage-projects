...
class SunRelativeInfo extends InterfaceElement {
  ...
  override protected def _update() {
    val ship_sun_vertical_speed = msecOrKmsec((player_ship.linearVelocity - sun.linearVelocity) * (player_ship.coord - sun.coord).n)
    val ship_sun_tangent_speed = msecOrKmsec(((player_ship.linearVelocity - sun.linearVelocity) * (player_ship.coord - sun.coord).p) / player_ship.coord.dist(sun.coord) * sun.radius - sun.groundSpeedMsec)
    ...
    val ship_sun_position = if(InterfaceHolder.degOrKm.selectedVariant == 0) {
      f"${correctAngle(DVec(0, 1).deg360(player_ship.coord - sun.coord) - sun.currentState.ang)}%.3f град."
    } else {
      f"${(correctAngle(DVec(0, 1).deg360(player_ship.coord - sun.coord) - sun.currentState.ang)/360.0*sun.length)/1000}%.2f км"
    }
