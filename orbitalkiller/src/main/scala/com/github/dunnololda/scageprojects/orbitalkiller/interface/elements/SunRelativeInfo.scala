...
class SunRelativeInfo extends InterfaceElement {
  ...
  override protected def _update() {
    ...
    val ship_sun_position = if(InterfaceHolder.degOrKm.selectedVariant == 0) {
      f"${correctAngle(DVec(0, 1).deg360(ship.coord - sun.coord) - sun.currentState.ang)}%.3f град."
    } else {
      f"${(correctAngle(DVec(0, 1).deg360(ship.coord - sun.coord) - sun.currentState.ang)/360.0*sun.length)/1000}%.2f км"
    }
